        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th scope="col">هل تم رفع المرفقات</th>
                </tr>
            </thead>
                <tr>
                <!-- Initial Studies -->
                <td>
                    @foreach(json_decode($attaches_infos->filename) as $image)
                        <a target="_blank" href="{{ asset('/attach/images/'.$image) }}">عرض المرفقات </a><br/>
                    @endforeach
                </td>
                    <td>{{$attaches_infos->take_image}}</td>
                </tr>
        </table>
    </div>
</div>
